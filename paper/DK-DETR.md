...
- 通过正弦encoding和投影层，这些锚框用于生成content queries $Q^{obj} = \{\mathbf{q}_1^{obj},\mathbf{q}_2^{obj},\ldots,\mathbf{q}_N^{obj}\} \in \mathbb{R}^{N\times D}$ （在DETR中被叫做object queries）和后边decoder的位置embedding
- 检测分支：
  - N个content queries、memory特征、位置embedding被输入到6个decoder层中，得到N个目标embeddings，得到N个潜在的目标特征 (object features)
  - N个目标特征使用投影层将目标特征和text embedding尺寸对齐，接着对齐后的目标特征被送入了text-based分类器来产生对应于base类别分类分数（用于训练），或者base+novel类别的分类分数（用于推理）
...
Text-based分类器的步骤：
- 受预训练视觉语言模型CLIP的启发，本文建议通过类别名称本身来定义标注空间，如将一个名字做成句子后送入到VLM text encoder来提取文本embedding t
- 再用embedding t去和目标的视觉特征f计算余弦相似度：
